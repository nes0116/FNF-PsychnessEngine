...
class ControlsSubState extends MusicBeatSubstate
{
	...
	var options:Array<Dynamic> = [
		...
		[false],
		[false, 'Open Console', 'debug_4', 'Open Console'],
		[false, 'Reload State', 'debug_5', 'Reload State']
	];
		...
		for (i => option in options)
		{
			if (onKeyboardMode || option[0])
			{
				if (option.length > 1)
				{
					...
					if (!isDefaultKey)
					{
						var newScale = Math.min(450 / text.width, 1);
						text.setScale(newScale, 1);
					}
	...
	override function update(elapsed:Float)
	{
		for (m in members)
		{
			if (Std.isOfType(m, FlxSprite))
				cast(m, FlxSprite).scrollFactor.set();
		}
