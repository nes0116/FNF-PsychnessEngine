...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.sound.music != null)
		{
			...
			if (holdingAlt && FlxG.mouse.wheel != 0)
			{
				var amount:Float = GRID_SIZE * FlxG.mouse.wheel;
				var objArray:Array<FlxSprite> = [
					gridBg,
					prevGridBg,
					nextGridBg,
					waveformSprite,
				...
				for (m in icons)
					objArray.push(m);
				for (m in events)
					objArray.push(m.eventText);
				for (m in behindRenderedNotes)
					objArray.push(m);
				for (m in curRenderedNotes)
					objArray.push(m);
				for (m in movingNotes)
					objArray.push(m);
				for (m in strumLineNotes)
					objArray.push(m);
				for (m in objArray)
					m.x += amount;
				scrollX += amount;
		...
		if (mouseX >= minX && mouseX < gridBg.x + gridBg.width)
		{
			var diffX:Float = mouseX - gridBg.x;
			...
			else if (FlxG.mouse.justPressed && !ignoreClickForThisFrame)
			{
				if (FlxG.keys.pressed.CONTROL && FlxG.mouse.justPressed)
				{
					if (selectedNotes.length > 0)
						...
						showOutput('[Move Notes] You must select notes to move them!', true);
				}
				else if (mouseX >= gridBg.x && mouseX < gridBg.x + gridBg.width)
				{
			...
			if (changed)
			{
				...
				if (Paths.fileExists(textureLoad, IMAGE) || noteTextureInputText.text.trim() == '')
				{
					...
					if (noteTextureInputText.text.trim().length > 0)
						showOutput('[Note Texture] Reloaded notes to: "$textureLoad"');
					else
						showOutput('[Note Texture] Reloaded notes to default texture');
				}
				else
					showOutput('[Note Texture] ERROR: "$textureLoad" not found.', true);
			}
		};
		...
		function genericEventButton(func:EventMetaNote->Void)
		{
			if (selectedNotes.length == 1)
			{
				if (selectedNotes[0].isEvent)
				{
					...
					showOutput('[Event] Note selected must be an Event!', true);
			}
			else
				showOutput('[Event] You must select a single event to press this button.', true);
		}
			...
			genericEventButton(function(event:EventMetaNote)
			{
				if (event.events.length > 1)
				{
					...
					if (selectedEvent != null)
					{
						...
						showOutput('[Remove Event] No event is selected when you deleted it?? Weird.', true);
				}
				else
				{
	...
	function addSectionTab()
	{
		...
		function copyNotesOnSection(?secOff:Int = 0, ?noteOff:Int = 0, ?showMessage:Bool = true) // Used on "Copy Section" and "Copy Last Section" buttons
		{
			...
			if (showMessage)
			{
				...
				{
					showOutput('[Note] Nothing to copy!', true);
					return;
				}
		...
		var duetSectionButton:PsychUIButton = new PsychUIButton(objX + 100, objY, 'Duet Section', function()
		{
			...
			for (note in curRenderedNotes.members)
			{
				...
				if (side > -1)
				{
					...
					{
						showOutput('[Duet Section] You cannot press this button with notes on more than one side.');
						return;
					}
				}
				else
			...
			?showMessage:Bool = true) // Used on "Paste Section" and "Copy Last Section" buttons
	{
		...
		if (showMessage)
		{
			...
			{
				showOutput('[Paste Note] Nothing to paste!', true);
				return [];
			}

			var str:String = '';
			if (nts.length > 0)
				str += '[Paste Note] Notes Added: ${nts.length}';
			if (evs.length > 0)
			{
				if (str.length > 0)
					str += '\n';
				str += '[Paste Event] Events Added: ${evs.length}';
			}
	...
	function addSongTab()
	{
		...
		var reloadJsonButton:PsychUIButton = new PsychUIButton(objX + 205, objY, 'Reload JSON', function()
		{
			...
			{
				showOutput('[Reload JSON] Error: File loaded is not a Psych Engine/FNF 0.2.x.x chart.', true);
				return;
			}
			...
			var func:Void->Void = function()
			{
				...
				reloadNotesDropdowns();
				prepareReload();
				showOutput('[Reload JSON] Opened chart "${diff ? curdiff : cur}" successfully!');
			}
		...
		var btn:PsychUIButton = new PsychUIButton(objX, objY, 'BPM Detection', function()
		{
			...
			if (_tapTimes.length >= 2)
			{
				...
				showOutput('[BPM Detection] Detected BPM: $bpm');
			}
		});
	...
	function addFileTab()
	{
			...
			openSubState(new BasePrompt(620, 380, 'Open Chart Data', function(state:BasePrompt)
			{
				...
				function _loadChart(data:String, path:String)
				{
					try
					{
						...
						if (loadedChart == null || !Reflect.hasField(loadedChart, 'song')) // Check if chart is ACTUALLY a chart and valid
						{
							showOutput('[Load Chart] Error: File loaded is not a Psych Engine/FNF 0.2.x.x chart.', true);
							return;
						}
						...
						var func:Void->Void = function()
						{
							...
							reloadNotesDropdowns();
							prepareReload();
							showOutput('[Load Chart] Opened chart "${Song.chartPath}" successfully!');
						}
					...
					{
						state.close();
						showOutput('[Load Chart] Error: ${e.message}', true);
						trace(e.stack);
					}
				}
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Open Autosave...', function()
		{
			...
			{
				showOutput('[Open Autosave] The "backups" folder does not exist.', true);
				return;
			}
			...
			{
				showOutput('[Open Autosave] No autosave files found.', true);
				return;
			}
				...
				var btn:PsychUIButton = new PsychUIButton(0, radioGrp.y + radioGrp.height + 20, 'Load', function()
				{
					...
					if (FileSystem.exists(path))
					{
						try
						{
							...
							{
								showOutput('[Load Autosave] Error: File loaded is not a valid Psych Engine autosave.', true);
								return;
							}
							...
							var func:Void->Void = function()
							{
								...
								showOutput('[Load Autosave] Opened autosave "$autosaveName" successfully!');
							}
						...
						{
							showOutput('[Load Autosave] Error on loading autosave: ${e.message}', true);
						}
					}
					else
						showOutput('[Load Autosave] Error! Autosave file selected could not be found, huh??', true);
				});
				btn.cameras = state.cameras;
				btn.screenCenter(X);
				state.add(btn);
			...
			var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Open Events...', function()
			{
				...
				fileDialog.open(function()
				{
					try
					{
						...
						{
							showOutput('[Load Events] Error: File loaded is not a Psych Engine chart/events file.', true);
							return;
						}
						...
						{
							showOutput('[Load Events] Events file loaded is empty.', true);
							return;
						}
						...
						openSubState(new BasePrompt('Events Found! Choose an action.', function(state:BasePrompt)
						{
							...
							var btn:PsychUIButton = new PsychUIButton(0, btnY, 'Replace All', function()
							{
								...
								softReloadNotes();
								state.close();
								showOutput('[Load Events] Events loaded successfully!');
							});
							...
							var btn:PsychUIButton = new PsychUIButton(0, btnY, 'Add', function()
							{
								...
								softReloadNotes();
								state.close();
								showOutput('[Load Events] Events added successfully!');
							});
							btn.screenCenter(X);
							btn.cameras = state.cameras;
							state.add(btn);
					...
					{
						showOutput('[Load Events] Error: ${e.message}', true);
						trace(e.stack);
					}
				});
			}, btnWid);
		...
		btn.description = 'Save as...\n- Save this chart to any location.';
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		...
		if (SHOW_EVENT_COLUMN)
		{
			...
			var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Save Events...', function()
			{
				...
				fileDialog.save('events.json', PsychJsonPrinter.print({events: PlayState.SONG.events, format: 'psychness_0.4.3'}, ['events']),
					function() showOutput('[Save Events] Events saved successfully to: ${fileDialog.path}'), null,
					function() showOutput('[Save Events] Error on saving events!', true));
			}, btnWid);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Reload Chart', function()
		{
			var func:Void->Void = function()
			{
				if (Song.chartPath == null)
				{
					showOutput('[Reload Chart] You must save/load a Chart first to Reload it!', true);
					return;
				}
				...
				if (FileSystem.exists(Song.chartPath))
				{
					try
					{
						...
						reloadNotesDropdowns();
						prepareReload();
						showOutput('[Reload Chart] Chart reloaded successfully!');
					}
					catch (e:Exception)
					{
						showOutput('[Reload Chart] Error: ${e.message}', true);
						trace(e.stack);
					}
				}
				else
		...
		btnY++;
		btnY += 20;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Save (Psych)...', function()
		{
			if (!fileDialog.completed)
				return;
			upperBox.isMinimized = true;
		...
		btn.description = 'Save (Psych)...\n- Save Chart for Psych Engine 1.x.\n- Shortcut: Ctrl + Shift + S';
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Save (V-Slice)...', function()
		{
			...
			fileDialog.openDirectory('Save V-Slice Chart/Metadata JSONs', function()
			{
								...
								function()
								{
										...
										PsychJsonPrinter.print(pack.metadata, ['characters', 'difficulties', 'timeChanges']), function()
									{
										if (overwriteSavedSomething)
											showOutput('[Save Chart for V-Slice] Files saved successfully to: $path!');
									});
								});
							state.close();
						});
				...
				{
					showOutput('[Save Chart for V-Slice] Error: ${e.message}', true);
					trace(e.stack);
				}
			});
		}, btnWid);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Psych to V-Slice...', function()
		{
			...
			fileDialog.open('song.json', 'Open a Psych Engine Chart JSON', function()
			{
				...
				if (loadedChart == null || !Reflect.hasField(loadedChart, 'song')) // Check if chart is ACTUALLY a chart and valid
				{
					showOutput('[Psych to V-Slice] Error: File loaded is not a Psych Engine 0.x.x/FNF 0.2.x.x chart.', true);
					return;
				}
				...
				{
					showOutput('[Psych to V-Slice] Error: Chart loaded is invalid.', true);
					return;
				}
							...
							if (diffs != null && diffs.length > 0)
							{
								...
								fileDialog.openDirectory('Save V-Slice Chart/Metadata JSONs', function()
								{
										...
										PsychJsonPrinter.print(pack.chart, ['events', 'notes', 'scrollSpeed']), function()
									{
										...
										{
											if (overwriteSavedSomething)
												showOutput('[Psych to V-Slice] Files saved successfully to: $path!');
										});
									});
								});
							}
							else
								showOutput('[Psych to V-Slice] Error: You need atleast one difficulty to export.', true);
						}
						catch (e:Exception)
						{
							showOutput('[Psych to V-Slice] Error: ${e.message}', true);
							trace(e.stack);
						}
						state.close();
					});
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  V-Slice to Psych...', function()
		{
			...
			fileDialog.open('chart.json', 'Open a V-Slice Chart file', function()
			{
				...
				{
					showOutput('[V-Slice to Psych] Error: File loaded is not a valid FNF V-Slice chart.', true);
					return;
				}
				...
				fileDialog.open('metadata.json', 'Open a V-Slice Metadata file', function()
				{
					...
					{
						showOutput('[V-Slice to Psych] Error: File loaded is not a valid FNF V-Slice metadata.', true);
						return;
					}
					...
					try
					{
						...
						if (pack.difficulties != null)
						{
							fileDialog.openDirectory('Save Converted Psych JSONs', function()
							{
								...
								function nextChart()
								{
									...
									if (pack.events != null)
									{
										...
										{
											if (overwriteSavedSomething)
												showOutput('[V-Slice to Psych] Files saved successfully to: ${fileDialog.path}!');
										}, true);
									}
							...
							showOutput('[V-Slice to Psych] Error: No difficulties found.');
					}
					catch (e:Exception)
					{
						showOutput('[V-Slice to Psych] Error: ${e.message}', true);
						trace(e.stack);
					}
				});
			});
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Update (Legacy)...', function()
		{
			...
			fileDialog.open(function()
			{
				...
				try
				{
					...
					if (loadedChart == null || !Reflect.hasField(loadedChart, 'song')) // Check if chart is ACTUALLY a chart and valid
					{
						showOutput('[Update (Legacy)] Error: File loaded is not a Psych Engine 0.x.x/FNF 0.2.x.x chart.', true);
						return;
					}
					...
					if (!fmt.startsWith('psych_v1'))
					{
						...
						showOutput('[Update (Legacy)] Updated "$filePath" from format "$fmt" to "psych_v1" successfully!');
					}
					else
						showOutput('[Update (Legacy)] Chart is already up-to-date! Format: "$fmt"', true);
				}
				catch (e:Exception)
				{
					showOutput('[Update (Legacy)] Error: ${e.message}', true);
					trace(e.stack);
				}
			});
		}, btnWid);
	...
	function addCharListTab()
	{
		...
		var charEditButton:PsychUIButton = new PsychUIButton(btnX, btnY, 'Edit', function()
		{
			...
			openSubState(new BasePrompt(270, 284, 'Edit Character', function(state:BasePrompt)
			{
				...
				{
					var oldVal:Bool = !visibleCheckBox.checked;
					if (PsychUIInputText.focusOn != null)
					{
						visibleCheckBox.checked = oldVal;
						return;
					}
				...
				{
					var oldVal:Bool = !strumVisibleCheckBox.checked;
					if (PsychUIInputText.focusOn != null)
					{
						strumVisibleCheckBox.checked = oldVal;
						return;
					}
				...
				{
					var oldVal:Bool = !noteVisibleCheckBox.checked;
					if (PsychUIInputText.focusOn != null)
					{
						noteVisibleCheckBox.checked = oldVal;
						return;
					}
				...
				{
					var oldVal:Float = characterData.position[0];
					if (PsychUIInputText.focusOn != null)
					{
						positionXStepper.value = oldVal;
						return;
					}
				...
				{
					var oldVal:Float = characterData.position[1];
					if (PsychUIInputText.focusOn != null)
					{
						positionYStepper.value = oldVal;
						return;
					}
				...
				{
					var oldVal:Float = characterData.strumPosition[0];
					if (PsychUIInputText.focusOn != null)
					{
						strumPositionXStepper.value = oldVal;
						return;
					}
				...
				{
					var oldVal:Float = characterData.strumPosition[1];
					if (PsychUIInputText.focusOn != null)
					{
						strumPositionYStepper.value = oldVal;
						return;
					}
				...
				{
					var oldVal:Float = characterData.index;
					if (PsychUIInputText.focusOn != null)
					{
						charIndexStepper.value = oldVal;
						return;
					}
		...
		var charDeleteButton:PsychUIButton = new PsychUIButton(btnX, btnY, 'Delete', function()
		{
			if (PlayState.SONG.characters[charListRadioGroup.checked].characterType == 'opponent')
			{
				...
				{
					showOutput('[Delete Character] Error: Cannot delete the only opponent character!', true);
					return;
				}
			}
			if (PlayState.SONG.characters[charListRadioGroup.checked].characterType == 'player')
			{
				...
				if (players.length == 1)
				{
					showOutput('[Delete Character] Error: Cannot delete the only player character!', true);
					return;
				}
			}
	...
	function saveChart(canQuickSave:Bool = true)
	{
		...
		if (canQuickSave && Song.chartPath != null)
		{
			...
			showOutput('[Save] Chart saved successfully to: ${Song.chartPath}');
		}
		else
		{
			...
			fileDialog.save(chartName, chartData, function()
			{
				...
				reloadNotesDropdowns();
				showOutput('[Save] Chart saved successfully to: $newPath');
			}, function()
			{
			...
			{
				chartSaved = oldChartSaved;
				showOutput('[Save] Error on saving chart!', true);
			});
		}
	}
	...
	function saveChartForPsych(canQuickSave:Bool = true)
	{
		...
		var charField:Array<{name:String, type:Int, charIndex:Int}> = [
			{name: '', type: 0, charIndex: 0},
			{name: '', type: 1, charIndex: 0},
			{name: 'gf', type: 2, charIndex: 0}
		];
		...
		Reflect.setField(songData, 'player1', charField[0].name);
		Reflect.setField(songData, 'player2', charField[1].name);
		Reflect.setField(songData, 'gfVersion', charField[2].name);
		...
		var chartData:String = PsychJsonPrinter.print(songData, ['sectionNotes', 'events']);

		if (canQuickSave && Song.chartPath != null)
		{
			pushToNearestList([Song.chartPath, Mods.currentModDirectory]);
			File.saveContent(Song.chartPath, chartData);
			showOutput('[Save for Psych 1.x] Chart saved successfully to: ${Song.chartPath}');
		}
		else
		{
			var oldChartSaved:Bool = chartSaved;
			var chartName:String = Paths.formatToSongPath(songData.song) + '.json';
			if (Song.chartPath != null)
				chartName = Song.chartPath.substr(Song.chartPath.lastIndexOf('/')).trim();
			fileDialog.save(chartName, chartData, function()
			{
				...
				pushToNearestList([newPath, Mods.currentModDirectory]);
				reloadNotesDropdowns();
				showOutput('[Save for Psych 1.x] Chart saved successfully to: $newPath');
			}, function()
			{
				chartSaved = oldChartSaved;
			}, function()
			{
				chartSaved = oldChartSaved;
				showOutput('[Save for Psych 1.x] Error on saving chart!', true);
			});
		}
	}
			...
			buttons: [
				{
					...
					arguments: "--cwd=\"" + Sys.getCwd() + "\"" + " --newChart=true",
					title: "New Chart",
					description: null
				}
			]
	...
	function undo()
	{
		...
		switch (action.action)
		{
			...
			case ADD_CHARACTER:
				...
				{
					showOutput('[Add Character] Error: ${e.message}', true);
					return;
				}
			case DELETE_CHARACTER:
				...
				{
					showOutput('[Delete Character] Error: ${e.message}', true);
					return;
				}
			case MOVE_CHARACTER:
				try
				...
				{
					showOutput('[Move Character] Error: ${e.message}', true);
					return;
				}
		}
